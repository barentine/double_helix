...
def test_g2a_separability():
    ...
    g2a_xy_2d = ndimage.convolve(test_im, g2a_2d_kernel, mode='nearest')
    ...
    g2a_x = ndimage.convolve1d(test_im, g2a_x_kernel, axis=0, mode='nearest')
    g2a_xy_sep = ndimage.convolve1d(g2a_x, g2a_y_kernel, axis=1, mode='nearest')
...
def test_g2b_separability():
    ...
    g2b_xy_2d = ndimage.convolve(test_im, g2b_2d_kernel, mode='nearest')
    ...
    g2b_x = ndimage.convolve1d(test_im, g2b_x_kernel, axis=0, mode='nearest')
    g2b_xy_sep = ndimage.convolve1d(g2b_x, g2b_y_kernel, axis=1, mode='nearest')
...
def test_g2c_separability():
    ...
    g2c_xy_2d = ndimage.convolve(test_im, g2c_2d_kernel, mode='nearest')
    ...
    g2c_x = ndimage.convolve1d(test_im, g2c_x_kernel, axis=0, mode='nearest')
    g2c_xy_sep = ndimage.convolve1d(g2c_x, g2c_y_kernel, axis=1, mode='nearest')
...
def test_h2a_separability():
    ...
    h2a_xy_2d = ndimage.convolve(test_im, h2a_2d_kernel, mode='nearest')
    ...
    h2a_x = ndimage.convolve1d(test_im, h2a_x_kernel, axis=0, mode='nearest')
    h2a_xy_sep = ndimage.convolve1d(h2a_x, h2a_y_kernel, axis=1, mode='nearest')
...
def test_h2b_separability():
    ...
    h2b_xy_2d = ndimage.convolve(test_im, h2b_2d_kernel, mode='nearest')
    ...
    h2b_x = ndimage.convolve1d(test_im, h2b_x_kernel, axis=0, mode='nearest')
    h2b_xy_sep = ndimage.convolve1d(h2b_x, h2b_y_kernel, axis=1, mode='nearest')
...
def test_h2c_separability():
    ...
    h2c_xy_2d = ndimage.convolve(test_im, h2c_2d_kernel, mode='nearest')
    ...
    h2c_x = ndimage.convolve1d(test_im, h2c_x_kernel, axis=0, mode='nearest')
    h2c_xy_sep = ndimage.convolve1d(h2c_x, h2c_y_kernel, axis=1, mode='nearest')
...
def test_h2d_separability():
    ...
    h2d_xy_2d = ndimage.convolve(test_im, h2d_2d_kernel, mode='nearest')
    ...
    h2d_x = ndimage.convolve1d(test_im, h2d_x_kernel, axis=0, mode='nearest')
    h2d_xy_sep = ndimage.convolve1d(h2d_x, h2d_y_kernel, axis=1, mode='nearest')
...
# import timeit
# n_runs = 1000
# DoubleGaussFit._USE_SEPARABLE_FILTERS = True
# # instantiate the kernels outside of the loop, time Detector.FilterFrame
# detector = DoubleGaussFit.Detector(md['Analysis.ROISize'])
# t_sep = timeit.timeit('detector.filter_frame(test_im)', globals=globals(), number=n_runs)

# DoubleGaussFit._USE_SEPARABLE_FILTERS = False
# detector = DoubleGaussFit.Detector(md['Analysis.ROISize'])
# t_2d = timeit.timeit('detector.filter_frame(test_im)', globals=globals(), number=n_runs)

# print(f'Separable filters: {t_sep:.3f} s for {n_runs} runs, 2D filters: {t_2d:.3f} s for {n_runs} runs')
